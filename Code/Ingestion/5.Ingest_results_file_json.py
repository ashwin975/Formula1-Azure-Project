...
# MAGIC %md
# MAGIC for race_id_list in results_final_df.select("race_id").distinct().collect():
# MAGIC     if spark._jsparkSession.catalog().tableExists("f1_processed.results"):
# MAGIC         spark.sql(f"ALTER TABLE f1_processed.results DROP IF EXISTS PARTITION (race_id = {race_id_list.race_id})")
...
# MAGIC %md
# MAGIC results_final_df.write.mode("append").partitionBy("race_id").format("parquet").saveAsTable("f1_processed.results")
